...
class TaskManager:
    SUPPORTED_COMMANDS = ("dataset.split", "training.start")
    ...
    def submit(
        ...
        self._append_log(task, "[WAIT ] REST API 명령 대기열에 등록되었습니다.")
        log_command_received(normalized_type, resolved_task_id, callback_url)

        with self._lock:
            self.tasks[resolved_task_id] = task

        self._send_callback(task, {"status": "accepted", "log": "[agent] command accepted\n"})
    ...
    def _run_task(self, task: AgentTask) -> None:
        ...
        log_command_execute(task.command_type, task.task_id)
        self._append_log(task, "[START] 명령 실행 시작")
        self._send_callback(task, {"status": "running", "log": "[agent] command running\n"})

        try:
            handler = self._resolve_handler(task.command_type)
            result = handler(task.payload, task)
            task.result = result
            task.status = "completed"
            task.finished_at = now_iso()
            self._append_log(task, "[ OK ] 명령 실행 완료")
            log_command_finished(task.task_id, "completed")
            self._send_callback(task, {
                "status": "completed",
                "result": result,
                "log": "[agent] command completed\n",
            })
            ...
            log_command_finished(task.task_id, "failed")
            self._send_callback(task, {
                "status": "failed",
                "errorMessage": str(error),
                "log": f"[agent] command failed: {error}\n",
            })

    def _resolve_handler(self, command_type: str) -> CommandHandler:
        if command_type == "dataset.split":
            return self._handle_dataset_split
        if command_type == "training.start":
            return self._handle_training_start
        raise ValueError(f"handler not found: {command_type}")

    def _handle_dataset_split(self, payload: dict[str, Any], task: AgentTask) -> dict[str, Any]:
        self._append_log(task, "[STEP] dataset split 실행")
        result = create_dataset_split(WORKSPACE_ROOT, payload)
        for line in result.get("logs") or []:
            self._append_log(task, line)
            self._send_callback(task, {"status": "running", "log": f"{line}\n"})
        return result

    def _handle_training_start(self, payload: dict[str, Any], task: AgentTask) -> dict[str, Any]:
        self._append_log(task, "[STEP] training job 시작")
    ...
    def _send_callback(self, task: AgentTask, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            log_callback(task.task_id, task.callback_url, str(payload.get("status") or "update"))
        except Exception as error:
            self._append_log(task, f"[callback-error] {error}")
            log_callback_error(task.task_id, error)
