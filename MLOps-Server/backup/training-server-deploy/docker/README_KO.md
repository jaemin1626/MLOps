...
| `docker logs -f intellivix-train-agent` | 위 `logs-docker.sh` 와 동일 |
...
```bash
TRAIN_IMAGE=intellivix-train-agent:1.0.0
TRAIN_CONTAINER=intellivix-train-agent
AGENT_PORT=9010
TRAIN_GPU_DEVICE=0          # GPU 없으면 빈 값
TRAIN_NETWORK_MODE=host
AUTH_TOKEN=                 # 필요 시 설정
```
...
docker exec intellivix-train-agent python -c "import torch; print(torch.cuda.is_available())"
```
